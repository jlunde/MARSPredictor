...
namespace MARS
{
    class Program
    {
        const int NumInput = 18;
        const int NumOutput = 1;
        const double CutoffThreshold = 0.0;
        const int xFold = 16;
        ...
        static void Main(string[] args)
        {
            ...
            var inputLines = File.ReadAllLines(@"C:\NFL_Data\2015\predictions_training.csv");
            var testLines = File.ReadAllLines(@"C:\NFL_Data\2015\predictions_test.csv");
            ...
            var testData = new double[testLines.Length][];
            ...
            {
                testData[i] = ExtractXValues(inputLines[i]);
            }
            ...
            else
            {
                ...
                var xValueData = inputData.Where((arr, index) => (index < lowIndex || index > highIndex)).ToArray();
                var tValueData = outputData.Where((arr, index) => (index < lowIndex || index > highIndex)).ToArray();
                var testValueData = testData.Where((arr, index) => (index < lowIndex || index > highIndex)).ToArray();
                ...
                {
                    var xValue = testData.Where((arr, index) => index == i).ToArray()[0];
                    var yValue = network.Compute(xValue);
